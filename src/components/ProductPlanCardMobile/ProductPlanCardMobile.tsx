    ...
    {coverages?.map((coverage: any, index: number) => (
      <div className="" key={index}>
        ...
        {coverage?.child_coverages?.map((child: any, index2: number) => (
          <div className={`${styles['']} px-4 my-2 d-flex align-items-center justify-content-between`} key={index2}>
            <p className={styles['tabDataTxt']}>{child?.name}</p>
            <div
              className={`${
                styles[child?.state ? 'boolImgContainerTick' : 'boolImgContainerCross']
  ...
  return (
    <div className="w-100 d-flex flex-column">
      ...
      >
        ...
        <div className={`${styles['dropImgContainer']} d-flex align-items-center justify-content-center`}>
          <Image alt="" src={showTab === data?.heading ? uparrow : dropDownIconRed} />
        </div>
      </div>
      {showTab === data?.heading && (
        <div className="w-100">
          {/* Coverage Tab Make Component In Future */}
          {data?.heading === 'Coverage' && <MobileCoverages coverages={coverages} />}
          {/* Add-ons Tab Make Component In Future */}
          {data?.heading === 'Add-ons' && (
            <div>
              {/* {console.log('allData?.PolicyAddons', allData?.PolicyAddons)} */}
              {allData?.PolicyAddons?.map((addon: any, index: number) => (
                <div className="px-3 my-2 d-flex align-items-center justify-content-between" key={index}>
  ...
  return (
    <div className={`my-5 ${styles['mainWrapper']}`}>
      ...
      <div className={`w-100 ${styles['wrapper']}`}>
        ...
        >
          <div className={`d-flex align-items-center justify-content-center ${styles['plusImgContainer']}`}>
            <Image alt="" src={ppCompareData.some(val => val.id === data?.id) ? minus : plus} />
          </div>
        ...
        >
          <div className={`w-100 d-flex align-items-center justify-content-between ${styles['']}`}>
            <div className={` ${styles['leftContainer']}`}>
              <div className={`d-flex align-items-center  ${styles['mainImgContainer']}`}>
          ...
          <div className={`w-100 d-flex align-items-end justify-content-between ${styles['']}`}>
            <div className={` ${styles['leftContainer']}`}>
        ...
        {showMore === index && (
          <div className={`mt-1 ${styles['moreDetailsContainer']}`}>
            {tabData.map((each, index2) => (
              <>
                <PPCardMobileTabEach key={index2} data={each} coverages={coverages} allData={data} />
        ...
        >
          <div className={`d-flex align-items-center justify-content-center ${styles['showMoreImgContainer']}`}>
            <Image alt="" src={showMore === index ? uparrow : dropDownIconRed} />
          </div>
          <p className={styles['showMoreTxt']}>{showMore === index ? 'Show Less' : 'More Details'}</p>
        </div>
      </div>
    </div>
  )
...
export default connect(mapStateToProps, mapDispatchProps)(ProductPlanCardMobile)
